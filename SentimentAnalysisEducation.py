...
def smart_score(base, q1, q2, strong):
    ...
    negA = ["not", "no", "never", "without", "can't", "cant","cannot", "don't", "dont", "wont", "won't", "n't"]
...
def labels(s): return "✅ DUPLICATE" if s>=TH else ("🤔 CLOSE MATCH" if s>=TH-0.05 else "❌ DIFFERENT")
def show_results(s):
    print(f"\n🎯 Result of Similarity: {round(s*100,1)}% [{bar(s)}] ➡️ {labels(s)}")
...
def run(q1,q2,title):
    ...
    show_results(s); show_flow(q1,q2)
